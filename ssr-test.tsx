...
import DetailPane from './src/ui/DetailPane';
import { Dep, fmtDate } from './src/types';
...
ok(html.includes(fmtDate(Date.now() + 10 * 86400000)) && html.includes('到期'), '刷新后有效豁免及到期日保留');
ok(html.includes('豁免已过期'), '刷新后过期豁免标记保留');

// 5) 详情面板：过期豁免按待复核参与流转
const NOW5 = Date.now();
const mkSsrDep = (over: Partial<Dep>): Dep => ({
  id: 1, name: 'x', version: '1.0.0', license: 'MIT', source: 'npm',
  projectId: null, status: 'pending', note: '', exemption: null, updatedAt: NOW5, ...over,
});
function transClass(h: string, label: string): string {
  const re = /<button class="(trans[^"]*)"[^>]*>([\s\S]*?)<\/button>/g;
  let m: RegExpExecArray | null;
  while ((m = re.exec(h))) { if (m[2].includes(label)) return m[1]; }
  return 'NOT_FOUND';
}
const noop = () => undefined;

const expiredPane = renderToString(createElement(DetailPane, {
...
const validPane = renderToString(createElement(DetailPane, {
...
const approvedPane = renderToString(createElement(DetailPane, {
