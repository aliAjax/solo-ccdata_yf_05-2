...
exports.histCommit = histCommit;
exports.histUndo = histUndo;
exports.histRedo = histRedo;
exports.actAddProject = actAddProject;
exports.actDeleteProject = actDeleteProject;
exports.actUpdatePolicy = actUpdatePolicy;
exports.actAddDep = actAddDep;
exports.actAssignDeps = actAssignDeps;
exports.actTransitionDeps = actTransitionDeps;
exports.actRemoveDeps = actRemoveDeps;
exports.actApplyImport = actApplyImport;
...
const DAY = 86400000;
function histCommit(h, next, label) {
    return {
...
function appendAudit(s, entries) {
    if (!entries.length)
        return s;
    let seq = s.seq;
    const audit = [...s.audit];
...
function actAddProject(s, name, kind, now) {
    const id = s.seq;
    const project = { id, name, policy: (0, types_1.clonePolicy)(kind), createdAt: now };
    let next = { ...s, seq: id + 1, projects: [...s.projects, project] };
    next = appendAudit(next, [{
            at: now, actor: ACTOR, action: 'project.create',
            detail: `新建项目「${name}」，套用${types_1.KIND_LABEL[kind]}政策`, projectId: id, depId: null,
        }]);
    return { next, label: `新建项目「${name}」`, message: `项目「${name}」已创建，政策：${types_1.KIND_LABEL[kind]}`, tone: 'ok' };
}
function actDeleteProject(s, projectId, now) {
    const project = s.projects.find(p => p.id === projectId);
    if (!project)
        return { next: null, label: '', message: '项目不存在', tone: 'error' };
    ...
    let next = { ...s, projects: s.projects.filter(p => p.id !== projectId), deps };
    next = appendAudit(next, [{
...
function actUpdatePolicy(s, projectId, policy, now) {
    const project = s.projects.find(p => p.id === projectId);
    if (!project)
        return { next: null, label: '', message: '项目不存在', tone: 'error' };
    let next = { ...s, projects: s.projects.map(p => p.id === projectId ? { ...p, policy } : p) };
    next = appendAudit(next, [{
            at: now, actor: ACTOR, action: 'policy.update',
            detail: `更新政策：${types_1.KIND_LABEL[policy.kind]}，禁止 ${policy.banned.length} 项许可，附加义务 ${policy.obligations.length} 条，例外期限 ${policy.exceptionDays} 天，生效范围 ${policy.scope.sources.join('/')}`,
            projectId, depId: null,
        }]);
    return { next, label: `更新「${project.name}」政策`, message: `「${project.name}」政策已更新，风险已按新政策重算`, tone: 'ok' };
}
function actAddDep(s, input, now) {
    const id = s.seq;
    const dep = {
    ...
    let next = { ...s, seq: id + 1, deps: [...s.deps, dep] };
...
function actAssignDeps(s, ids, projectId, now) {
    ...
    if (target) {
        ...
        for (const d of moving) {
            const r = (0, types_1.computeRisk)(d, target.policy);
            if (r === null)
                c.na++;
            else
                c[r === 'high' ? 'high' : r === 'medium' ? 'medium' : 'low']++;
        }
        const parts = [
            c.high ? `高风险 ${c.high}` : '',
...
function actTransitionDeps(s, ids, target, exemption, now) {
    const idSet = new Set(ids);
    // 豁免参数对整批统一校验
    ...
    for (const d of s.deps) {
        if (!idSet.has(d.id))
            continue;
        const from = (0, types_1.effectiveStatus)(d, now);
        const err = (0, types_1.transitionError)(from, target) ?? exemptErr;
        if (err)
            blocked.push({ dep: d, reason: err });
        else
            applied.push({ dep: d, from });
    }
    if (!applied.length) {
    ...
    const daysFor = (d) => s.projects.find(p => p.id === d.projectId)?.policy.exceptionDays ?? 30;
    const resolveExemption = (d) => ({
    ...
    next = appendAudit(next, applied.map(({ dep: d, from }) => {
        const fromLabel = (0, types_1.exemptionExpired)(d, now) ? '豁免已过期' : types_1.STATUS_LABEL[from];
        const ex = target === 'exempted' ? resolveExemption(d) : null;
        return {
            at: now, actor: ACTOR, action: 'dep.transition',
            detail: `「${fromLabel}」→「${types_1.STATUS_LABEL[target]}」${ex ? `，豁免至 ${(0, types_1.fmtDate)(ex.until)}：${ex.reason}` : ''}`,
            projectId: d.projectId, depId: d.id,
        };
    }));
    let message = `已流转 ${applied.length} 项 → ${types_1.STATUS_LABEL[target]}`;
    if (target === 'exempted' && exemption) {
        message += exemption.until !== null ? `，统一截止 ${(0, types_1.fmtDate)(exemption.until)}` : '，按各项目例外期限分别生效';
    }
    let tone = 'ok';
    if (blocked.length) {
    ...
    return { next, label: `批量流转 ${applied.length} 项 → ${types_1.STATUS_LABEL[target]}`, message, tone };
}
function actRemoveDeps(s, ids, now) {
    const idSet = new Set(ids);
    const removed = s.deps.filter(d => idSet.has(d.id));
    if (!removed.length)
        return { next: null, label: '', message: '没有选中的依赖', tone: 'warn' };
    let next = { ...s, deps: s.deps.filter(d => !idSet.has(d.id)) };
...
function actApplyImport(s, analysis, now) {
    const snap = analysis.snapshot;
    if (!analysis.ok || !snap)
        return { next: null, label: '', message: '导入分析无效', tone: 'error' };
    let seq = s.seq;
    const pid = seq++;
    const project = { id: pid, name: snap.project.name, policy: snap.project.policy, createdAt: now };
    const nameToId = new Map();
    const newDeps = snap.deps.map(d => {
        const id = seq++;
        nameToId.set(d.name, id);
        return { ...d, id, projectId: pid };
    });
    const importedAudit = snap.audit.map(a => ({
...
function seedStore() {
    const now = Date.now();
    let seq = 1;
    const projects = SEED_PROJECTS.map(([name, kind]) => ({
        id: seq++, name, policy: (0, types_1.clonePolicy)(kind), createdAt: now - 30 * DAY,
    }));
    const deps = SEED_DEPS.map(([name, version, license, source, pidx, status, exemptDays]) => {
    ...
    const audit = projects.map(p => ({
        ...
        detail: `新建项目「${p.name}」，套用${types_1.KIND_LABEL[p.policy.kind]}政策`, projectId: p.id, depId: null,
    }));
    return { seq, projects, deps, audit };
}
// ---------- 旧版迁移 ----------
function migrateOld(oldDeps) {
    const now = Date.now();
    let seq = 1;
    const project = { id: seq++, name: 'Aurora Web', policy: (0, types_1.clonePolicy)('closed'), createdAt: now };
    const statusMap = { ok: 'approved', warn: 'pending', risk: 'pending' };
    const deps = oldDeps
...
function useWorkbench() {
    const [hist, setHist] = (0, react_1.useState)(() => ({ past: [], future: [], current: loadStore() }));
    ...
    const run = (r) => {
        if (r.next)
            setHist(h => histCommit(h, r.next, r.label));
    ...
    const exportSnapshot = (projectId) => {
        const project = store.projects.find(p => p.id === projectId);
        if (!project)
            return;
        const deps = store.deps.filter(d => d.projectId === projectId);
        const snapshot = (0, snapshot_1.buildSnapshot)(project, deps, store.audit);
    ...
    return {
        ...
        deleteProject: (projectId) => run(actDeleteProject(store, projectId, Date.now())),
        updatePolicy: (projectId, policy) => run(actUpdatePolicy(store, projectId, policy, Date.now())),
        addDep: (input) => run(actAddDep(store, input, Date.now())),
        assignDeps: (ids, projectId) => run(actAssignDeps(store, ids, projectId, Date.now())),
        transitionDeps: (ids, target, exemption) => run(actTransitionDeps(store, ids, target, exemption, Date.now())),
        removeDeps: (ids) => run(actRemoveDeps(store, ids, Date.now())),
        exportSnapshot,
        applyImport: (analysis) => {
            const r = actApplyImport(store, analysis, Date.now());
            run(r);
            return r.next ? r.next.projects[r.next.projects.length - 1].id : null;
        },
        existingDepKeys: () => new Set(store.deps.map(d => (0, types_1.depKey)(d.name, d.version))),
