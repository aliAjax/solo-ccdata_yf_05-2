...
const snapshot_1 = require("./src/snapshot");
...
// ---- 流转表 ----
const all = ['pending', 'approved', 'exempted', 'rejected'];
for (const s of all)
    ok(!!(0, types_1.transitionError)(s, s)?.includes('重复'), `重复流转被拦截 ${s}`);
ok(!!(0, types_1.transitionError)('approved', 'exempted')?.includes('非法'), 'approved→exempted 非法');
ok(!!(0, types_1.transitionError)('rejected', 'approved')?.includes('非法'), 'rejected→approved 非法');
ok(!!(0, types_1.transitionError)('rejected', 'exempted')?.includes('非法'), 'rejected→exempted 非法');
eq((0, types_1.transitionError)('pending', 'approved'), null, 'pending→approved 合法');
eq((0, types_1.transitionError)('pending', 'exempted'), null, 'pending→exempted 合法');
eq((0, types_1.transitionError)('pending', 'rejected'), null, 'pending→rejected 合法');
eq((0, types_1.transitionError)('approved', 'pending'), null, 'approved→pending 合法');
eq((0, types_1.transitionError)('approved', 'rejected'), null, 'approved→rejected 合法');
eq((0, types_1.transitionError)('exempted', 'approved'), null, 'exempted→approved 合法');
eq((0, types_1.transitionError)('rejected', 'pending'), null, 'rejected→pending 合法');
eq(Object.keys(types_1.TRANSITIONS).length, 4, '流转表覆盖全部状态');
// ---- 风险重算 ----
const closed = (0, types_1.clonePolicy)('closed');
ok(closed.banned.includes('GPL-3.0'), '闭源政策禁止 GPL-3.0');
eq((0, types_1.computeRisk)(mkDep({ license: 'GPL-3.0' }), closed), 'high', '禁止许可=高风险');
eq((0, types_1.computeRisk)(mkDep({ license: 'LGPL-3.0' }), closed), 'medium', 'duty 义务=需关注');
eq((0, types_1.computeRisk)(mkDep({ license: 'MIT' }), closed), 'low', 'notice 义务=低风险');
eq((0, types_1.computeRisk)(mkDep({ license: 'ISC' }), closed), 'low', '无义务=低风险');
eq((0, types_1.computeRisk)(mkDep({ source: '镜像' }), { ...closed, scope: { sources: ['npm'] } }), null, '不在生效范围=不适用');
eq((0, types_1.computeRisk)(mkDep({}), null), null, '未归入项目=不适用');
const internal = (0, types_1.clonePolicy)('internal');
eq((0, types_1.computeRisk)(mkDep({ license: 'GPL-3.0' }), internal), 'medium', '内部政策下 GPL-3.0 仅提示义务');
eq((0, types_1.computeRisk)(mkDep({ license: 'SSPL-1.0' }), internal), 'high', '内部政策禁止 SSPL');
// ---- 过期豁免 ----
const expiredDep = mkDep({ status: 'exempted', exemption: { reason: 'r', until: NOW - 1000 } });
ok((0, types_1.exemptionExpired)(expiredDep, NOW), '过期豁免判定');
eq((0, types_1.effectiveStatus)(expiredDep, NOW), 'pending', '过期豁免视同待复核');
const validDep = mkDep({ status: 'exempted', exemption: { reason: 'r', until: NOW + 1000 } });
ok(!(0, types_1.exemptionExpired)(validDep, NOW), '有效豁免未过期');
eq((0, types_1.effectiveStatus)(validDep, NOW), 'exempted', '有效豁免保持豁免状态');
// ---- 快照导出/导入 ----
...
const snap = (0, snapshot_1.buildSnapshot)(project, deps, auditSrc);
eq(snap.format, 'license-workbench-snapshot', '快照格式标识');
eq(snap.version, snapshot_1.SNAPSHOT_VERSION, '快照版本');
...
eq((0, snapshot_1.analyzeSnapshot)('not json', { projectNames: [], depKeys: new Set() }).ok, false, '非 JSON 被拒绝');
eq((0, snapshot_1.analyzeSnapshot)('{}', { projectNames: [], depKeys: new Set() }).ok, false, '缺少格式标识被拒绝');
const tooNew = (0, snapshot_1.analyzeSnapshot)(JSON.stringify({ ...snap, version: 99 }), { projectNames: [], depKeys: new Set() });
eq(tooNew.ok, false, '过新版本被拒绝');
ok((tooNew.error ?? '').includes('v99'), '版本错误信息含版本号');
const clean = (0, snapshot_1.analyzeSnapshot)(JSON.stringify(snap), { projectNames: [], depKeys: new Set() });
eq(clean.ok, true, '干净快照可导入');
eq(clean.conflicts.length, 0, '干净快照无冲突');
eq(clean.depCount, 2, '导入依赖数');
const dirty = (0, snapshot_1.analyzeSnapshot)(JSON.stringify({
    ...snap,
    deps: [
        ...snap.deps,
...
const allDup = (0, snapshot_1.analyzeSnapshot)(JSON.stringify(snap), { projectNames: [], depKeys: new Set(['react@18.3.1', 'legacy-parser@2.1.0']) });
eq(allDup.ok, false, '全部重复时拒绝导入');
// ---- 历史栈 ----
const s0 = (0, store_1.seedStore)();
let h = { past: [], future: [], current: s0 };
const r1 = (0, store_1.actAddProject)(s0, 'Test Proj', 'open', NOW);
h = (0, store_1.histCommit)(h, r1.next, r1.label);
eq(h.current.projects.length, s0.projects.length + 1, '提交后项目+1');
eq(h.past.length, 1, '历史+1');
const u1 = (0, store_1.histUndo)(h);
eq(u1.hist.current.projects.length, s0.projects.length, '撤销恢复项目数');
eq(u1.label, '新建项目「Test Proj」', '撤销返回标签');
const r2 = (0, store_1.histRedo)(u1.hist);
eq(r2.hist.current.projects.length, s0.projects.length + 1, '重做恢复项目+1');
eq((0, store_1.histUndo)({ past: [], future: [], current: s0 }), null, '空历史撤销返回 null');
eq((0, store_1.histRedo)({ past: [], future: [], current: s0 }), null, '空未来重做返回 null');
// ---- 批量流转：合法/重复/非法混合 ----
const seed = (0, store_1.seedStore)();
...
const tr = (0, store_1.actTransitionDeps)(seed, mixed, 'approved', undefined, NOW);
...
const allBlocked = (0, store_1.actTransitionDeps)(seed, [approvedIds[0]], 'approved', undefined, NOW);
eq(allBlocked.next, null, '全部拦截时不产生新状态');
eq(allBlocked.tone, 'error', '全部拦截提示错误');
ok(allBlocked.message.includes('重复流转'), '全拦截消息含原因');
const noExempt = (0, store_1.actTransitionDeps)(seed, pendingIds, 'exempted', undefined, NOW);
eq(noExempt.next, null, '缺少豁免信息时拦截');
const withExempt = (0, store_1.actTransitionDeps)(seed, pendingIds, 'exempted', { reason: '测试', until: NOW + 86400000 }, NOW);
...
let h2 = { past: [], future: [], current: seed };
const bigAssign = (0, store_1.actAssignDeps)(seed, allIds, aurora.id, NOW);
h2 = (0, store_1.histCommit)(h2, bigAssign.next, bigAssign.label);
...
ok(seed.deps.some(d => (0, types_1.exemptionExpired)(d, Date.now())), '种子含过期豁免');
ok(seed.deps.some(d => d.status === 'exempted' && !(0, types_1.exemptionExpired)(d, Date.now())), '种子含有效豁免');
ok(seed.deps.some(d => d.projectId === null), '种子含未归入依赖');
const ids = new Set();
let seqOk = true;
for (const list of [seed.projects, seed.deps, seed.audit]) {
    for (const x of list) {
        if (ids.has(x.id) || x.id >= seed.seq)
            seqOk = false;
        ids.add(x.id);
    }
}
...
// ---- 删除/添加依赖 ----
const rm = (0, store_1.actRemoveDeps)(seed, allIds.slice(0, 5), NOW);
eq(rm.next.deps.length, seed.deps.length - 5, '批量删除依赖');
const add = (0, store_1.actAddDep)(seed, { name: 'new-lib', version: '1.0.0', license: 'MIT', source: 'npm', projectId: null }, NOW);
eq(add.next.deps.length, seed.deps.length + 1, '添加依赖');
eq(add.next.deps[add.next.deps.length - 1].status, 'pending', '新依赖默认待复核');
// ---- 过期豁免按待复核参与流转 ----
const DAY_MS = 86400000;
const T = Date.now();
const expired1 = seed.deps.find(d => (0, types_1.exemptionExpired)(d, T));
eq(expired1.status, 'exempted', '过期项原始状态仍为豁免');
eq((0, types_1.effectiveStatus)(expired1, T), 'pending', '过期项有效状态为待复核');
const dupPending = (0, store_1.actTransitionDeps)(seed, [expired1.id], 'pending', undefined, T);
eq(dupPending.next, null, '过期项→待复核视为重复被拦截');
ok(dupPending.message.includes('重复'), '过期项→待复核提示重复');
const expApprove = (0, store_1.actTransitionDeps)(seed, [expired1.id], 'approved', undefined, T);
ok(expApprove.next !== null, '过期项可直接批准');
// 单项重豁免：状态、到期日、记录一致
const expiredProj = seed.projects.find(p => p.id === expired1.projectId);
const re = (0, store_1.actTransitionDeps)(seed, [expired1.id], 'exempted', { reason: '重新申请豁免', until: null }, T);
...
ok(!(0, types_1.exemptionExpired)(reDep, T), '重豁免后不再过期');
...
ok(reAudit.detail.includes('重新申请豁免') && reAudit.detail.includes((0, types_1.fmtDate)(reDep.exemption.until)), '重豁免记录含理由与到期日');
// ---- 混合项目批量豁免：按各自政策分别生效 ----
...
const mixedBatch = (0, store_1.actTransitionDeps)(seed, [m0.id, m1.id, m2.id], 'exempted', { reason: '混合批量', until: null }, T);
ok(mixedBatch.next !== null, '混合项目批量豁免成功');
const untilOf = (id) => mixedBatch.next.deps.find(d => d.id === id).exemption.until;
...
ok(mixedAudits.find(a => a.depId === m0.id).detail.includes((0, types_1.fmtDate)(untilOf(m0.id))), '记录含闭源项目到期日');
ok(mixedAudits.find(a => a.depId === m1.id).detail.includes((0, types_1.fmtDate)(untilOf(m1.id))), '记录含内部项目到期日');
ok(mixedAudits.find(a => a.depId === m2.id).detail.includes((0, types_1.fmtDate)(untilOf(m2.id))), '记录含开源项目到期日');
// 统一截止日期仍覆盖整批
const uni = (0, store_1.actTransitionDeps)(seed, [m0.id, m1.id], 'exempted', { reason: '统一', until: T + 7 * DAY_MS }, T);
ok(uni.next.deps.every(d => ![m0.id, m1.id].includes(d.id) || d.exemption.until === T + 7 * DAY_MS), '统一截止日期覆盖整批');
// 无效截止时间拦截
const badUntil = (0, store_1.actTransitionDeps)(seed, [m0.id], 'exempted', { reason: 'x', until: T - 1000 }, T);
eq(badUntil.next, null, '过去截止时间被拦截');
eq(badUntil.tone, 'error', '无效截止时间提示错误');
ok(badUntil.message.includes('截止时间'), '无效截止时间消息含原因');
const badUntilBatch = (0, store_1.actTransitionDeps)(seed, [m0.id, m1.id], 'exempted', { reason: 'x', until: T - 1000 }, T);
eq(badUntilBatch.next, null, '批量中无效截止时间整批拦截');
// 未归入项目按默认 30 天
const unassigned = seed.deps.find(d => d.projectId === null);
const unRes = (0, store_1.actTransitionDeps)(seed, [unassigned.id], 'exempted', { reason: 'x', until: null }, T);
